...
namespace PingPong.Applications {
    public class PingApp : IApplication<PingDataReadyEventArgs> {
        ...
        public double TargetBounceHeight {
            set {
                lock (settingsSyncLock) {
        ...
        public PingApp(Robot robot, OptiTrackSystem optiTrack, Func<Vector<double>, bool> checkFunction) {
            ...
            regZ = new PIRegulator(0.7, 2, 1000);
        ...
        public void Stop() {
            if (isStarted) {
                ...
                Task.Run(() => {
                    robot.ForceMoveTo(robot.HomePosition, RobotVector.Zero, 0.7);
        ...
        private void ProcessOptiTrackFrame(object sender, OptiTrack.FrameReceivedEventArgs args) {
            ...
            samplesBufferForMaxHeightDetection.Add(ballPosition);

            if (samplesBufferForMaxHeightDetection.Count == 6) {
                samplesBufferForMaxHeightDetection.RemoveAt(0);

                bool bounceHeightFound = true;

                for (int i = 1; i < 3; i++) {
                    if (samplesBufferForMaxHeightDetection[i][2] <= samplesBufferForMaxHeightDetection[i - 1][2]) {
                ...
                if (bounceHeightFound) {
                    currentBounceHeight = samplesBufferForMaxHeightDetection[2][2];
                    data.LastBounceHeight = currentBounceHeight;
                    samplesBufferForMaxHeightDetection.Clear();
                }
            }
            ...
            if (prediction.IsReady) {
                if (prediction.TimeToHit >= 0.25) {
                    ...
                    double speedZ = 450.0;
                    speedZ = (Math.Sqrt(2.0 * 9.81 * (regZ.SetPoint - 180.0) * 1000) + prediction.Velocity[2] * 0.78) / (1 + 0.78);
                    speedZ += regZ.ComputeU(currentBounceHeight);
                    speedZ = Math.Min(Math.Max(speedZ, 180.0), 700.0);
                    if (speedZ == 180.0 || speedZ == 700.0) {
                        Console.WriteLine("Nasycenie speeda: " + speedZ);
                    }
                    //Console.WriteLine("Speed: " + speedZ + " w tym reg: " + regZ.ComputeU(currentBounceHeight) + " H: " + currentBounceHeight);
                    ...
                    lock (settingsSyncLock) {
                        regZ.SetParams(regZ.Kp, regZ.Ki, targetBounceHeight);
                    }
                    regB.Shift();
                    regC.Shift();
                    regZ.Shift();
